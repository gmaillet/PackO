...
def export_as_json(filename, jobs):
...
def generate(update):
    ...
    try:
        ...
        if not args.running:
            export_as_json(args.cache + '/create_graph.json', cmds2)
    ...
    # on commence pas initialiser uniquement avec le graph
    # il faut récupérer la liste des images dans le graph ainsi que la bbox
